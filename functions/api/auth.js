...
async function login(request, env) {
    ...
    */
    ...
    if (
        ...
        console.error(
            "Authentication secrets belum dikonfigurasi."
        );
    ...
    */
    ...
    if (!env.DB) {
        ...
        console.error(
            "Binding DB tidak tersedia."
        );

        return jsonResponse(
            {
                success: false,
                message: "Database autentikasi tidak tersedia."
            },
            500
        );
    }
    ...
    /*
    |--------------------------------------------------------------------------
    | CEK CREDENTIAL BOOTSTRAP
    |--------------------------------------------------------------------------
    |
    | ADMIN_USERNAME + ADMIN_PASSWORD dari Cloudflare
    | dianggap sebagai credential utama untuk akun admin.
    |
    | Jika cocok:
    |
    | - admin belum ada → buat
    | - admin sudah ada → sinkronkan password
    |
    */
        ...
        */
        ...
        if (!admin) {
            ...
            try {
                ...
                /*
                 * Jika ada request bersamaan yang lebih dulu
                 * membuat admin, ambil kembali akun tersebut.
                 */
        ...
        */
        ...
        if (!currentAdmin) {
            ...
            return jsonResponse(
                {
                    success: false,
                    message: "Akun admin gagal dibuat."
                },
                500
            );
        }
        ...
        |
        | Kalau ADMIN_PASSWORD di Cloudflare diganti,
        | password hash di D1 ikut diperbarui.
        |
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | LOGIN DENGAN ADMIN ENV
        |--------------------------------------------------------------------------
        */

        return await createLoggedInResponse(
            {
                ...currentAdmin,
    ...
    |--------------------------------------------------------------------------
    |
    | Kalau credential yang dimasukkan bukan credential ENV,
    | kita tetap mendukung akun admin yang tersimpan di D1.
    |
    */
...
async function createLoggedInResponse(
    admin,
    env
) {
...
async function getCurrentUser(
    ...
    /*
     * Pastikan admin masih ada dan aktif.
     */
